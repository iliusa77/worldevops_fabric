...
host = '104.236.30.84'
dump = True  # restore database from dump
...
def client():
    env.hosts = [host]
    env.user = 'rfq'
    env.password = 'xcNjlvVF'
    env.project = 'choice_portal'
    ...
    env.site = {
        'domain': 'choicerfq.com',  # without www.
        'docroot': env.docroot,
        'ssl': False,
        'port': 80,
        'login': 'admin',
        'admin': 'admin@choicerfq.com',
        'admin_pwd': genpass()
    }
...
def setup():
    ...
    fabtools.require.deb.packages([
        ...
        'python-pil', 'python-nose', 'libgeos-dev'
    ])
...
def restore_from_dump():
    source = Path('./', 'app-mysql', 'dump.sql')

    with settings(abort_exception=FabricException):
        try:
            run('mkdir {}'.format(env.downloads))
        except FabricException:
            pass
    target = Path(env.downloads)
    put(source, env.downloads)
    env.db = {
        'name': 'choice_portal',
        'user': 'choice_portal',
        'pass': 'KzTiHJUc',
        'dump': Path(target, 'dump.sql')
    }
    # env.db['dump'] = target
    run('mysql -u{user} -p{pass} {name} < {dump}'.format(**env.db))
...
Droplet Name: choicerfq.com
IP Address: 104.236.30.84
Username: root
Password: cb889d3393b0856d

user rfq
password  xcNjlvVF

==================== Site ====================
domain: choicerfq.com
port: 80
admin: admin@choicerfq.com
admin_pwd: admin
ssl: False
docroot: /home/rfq/projects/choice_portal/choice_portal
login: admin

================== Database ====================
mysql
name: choice_portal
url: mysql://choice_portal:KzTiHJUc@localhost:3306/choice_portal
host: localhost
user: choice_portal
pass: KzTiHJUc
root: VvRCBmHo
