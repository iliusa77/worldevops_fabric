var App = function () {
	...
	var handleRaty = function () {
		$.fn.raty.defaults.path = '{{STATIC_URL}}js/jquery-raty/img';
		...
		$('#starHalf-demo').raty({
			path    : '{{STATIC_URL}}js/jquery-raty/img',
			half    : true,
	...
	var handleJqgrid = function () {
		...
		jQuery("#rowed3").jqGrid({
			...
			datatype: "local",
			height: 350,
			colNames: ['RFQ No.', 'User', 'Office', 'Location', 'Packer', 'Customer','Destination','Date Quoted','Status','Valid Date', 'Cont.No','Action'],
			colModel: [{
				name: 'RFQ No',
				index: 'RFQ No',
				width: 20,
	...
	var handleTimeline = function () {
		createStoryJS({
			...
			width:		'100%',
			height:		'600',
			source:		'{{STATIC_URL}}js/timelinejs/example_json.json',
			embed_id:	'my-timeline',
			debug:		true,
			css:        '{{STATIC_URL}}js/timelinejs/css/timeline.css',     
            js:         '{{STATIC_URL}}js/timelinejs/js/timeline-min.js'    
		});
	}
	/*-----------------------------------------------------------------------------------*/
	/*	Handle Slidernav
		...
		if ($.cookie('gritter_show')) {
            ...
            setTimeout(function () {
                var unique_id = $.gritter.add({
                    ...
                    image: '{{STATIC_URL}}img/gritter/cloud.png',
            ...
            setTimeout(function () {
                ...
                var unique_id = $.gritter.add({
                    ...
                    // (string | optional) the image to display on the left
                    image: '{{STATIC_URL}}img/gritter/buy.png',
                    // (bool | optional) if you want it to fade out on its own or just sit there
                    sticky: true,
                    // (int | optional) the time you want it to be alive for before fading out
                    time: '',
	...
	return {
        ...
        blockUI: function (el, loaderOnTop) {
            lastBlockedUI = el;
            jQuery(el).block({
                message: '<img src="{{STATIC_URL}}img/loaders/12.gif" align="absmiddle">',
                css: {
                    border: 'none',
                    padding: '2px',
