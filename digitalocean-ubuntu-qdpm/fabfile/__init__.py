...
def chmod():
    with cd(env.project_dir):
        run('chmod -R 777 core/cache core/config core/log uploads/attachments uploads/users')
...
def setup_project():
    ...
    with cd(env.project_dir):
        run('wget -O source.zip http://downloads.sourceforge.net/project/qdpm/qdPM_9.0.zip')
...
def report():
    print(green('You can access your site by URL http://%s/ and continue installation in browser using database credentials below' % env.domain))
    print(green('------------------------------------'))
    ...
    print('Database port leave blank')
