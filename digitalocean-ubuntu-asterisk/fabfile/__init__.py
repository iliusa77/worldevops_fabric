...
def update_system():
    ...
    put('./grubpc.debconf', '/tmp/grubpc.debconf')
    sudo('cat /tmp/grubpc.debconf | debconf-set-selections')
    sudo(
        "sed -i.bak 's/http:\/\/us\.archive\.ubuntu\.com\/ubuntu\//http:\/\/bouyguestelecom\.ubuntu\.lafibre\.info\/ubuntu\//g' /etc/apt/sources.list")
...
def setup_users_groups():
    user = "asteriskpbx"
    fabtools.group.exists(user)
    fabtools.user.create(name=user, password="password", shell='/bin/zsh')
...
def install_asterisk_module_dependencies():
    ...
    sudo("apt-get install %s -f -y" % list)
